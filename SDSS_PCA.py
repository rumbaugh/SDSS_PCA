...
class SDSS_PCA:
    ...
    def cut_master(self, imax):
        ...
        if imax > len(self.master): 
            print 'imax greater than length of master'
    ...
    def prune_master(self, spec_dir='./spec_dir/'):
        ...
        plates = self.master.plate.values
        mjds   = self.master.mjd.values
        fibers = self.master.fiberid.values
        ids    = self.master.specobjid.values
    ...
    def load_spec_files(self, spec_dir='./spec_dir', smooth_wid=10, wavstep=None, wavmin=0, wavmax=10000, savefile=None):
        ...
        redshifts = self.master.z.values
        plates    = self.master.plate.values
        mjds      = self.master.mjd.values
        fibers    = self.master.fiberid.values
        ids       = self.master.specobjid.values
    ...
    def do_pca(self, n_components='mle'):
    ...
    def nnclassify(self, target=None, train_perc=0.9, n_neighbors=5,  algorithm='kd_tree', weights='distance'):
        ...
        train_X = train_df
        train_y = target[:int(train_perc * len(self.flux_pca))]
        test_X = self.flux_pca[int(train_perc * np.shape(self.flux_pca)[0]):]
        test_y = target[int(train_perc * np.shape(self.flux_pca)[0]):]
        self._clf = neighbors.KNeighborsClassifier(n_neighbors, weights=weights, algorithm=algorithm)
        self._clf.fit(train_X, train_y)
        self.predicted_y = self._clf.predict(test_X)
        self.cv_scores = cross_val_score(self._clf, self.flux_pca, target, cv=self.nfolds)
    
    def rfclassify(self, target=None, train_perc=0.9, n_estimators=10):
    ...
    def show_cross_val(self, nsigma=1):
        print "Accuracy: %.3f +/- %.3f"%(self.cv_scores.mean(), self.cv_scores.std() * nsigma)
        
    def compare_predictions(self, target=None, check_values=None, imax=None, verbose=False, returnperc=False):
        ...
        if check_values is None:
            check_values = target[-len(self.predicted_y):]
        if not(verbose):
            ...
            if imax is None: 
                imax = len(self.predicted_y)
            if imax > masterlen:
                imax = masterlen
            for i in range(0, imax):
                print check_values[i], self.predicted_y[i]
            
    def plot_pca_component(self, component, doshow=False, clear=True, fignum=None):
    ...
    def plot_pca_decomp(self, lightcurve, max_components=None, savefile=None, colors=['cyan', 'blue', 'magenta', 'red', 'pink', 'orange', 'yellow', 'green', 'gray', 'brown', 'purple', 'silver'], fignum=None):
        ...
        if max_components == None: 
            max_components = np.shape(self.pca.components_)[0]
        if max_components > np.shape(self.pca.components_)[0]:
            max_components = np.shape(self.pca.components_)[0]
    ...
    def plot_pca_model(self, lightcurve, max_components=None, savefile=None, colors=['cyan', 'blue', 'magenta', 'red', 'pink', 'orange', 'yellow', 'green', 'gray', 'brown', 'purple', 'silver'], fignum=None, plottype=True, ignore_components=[]):
    ...
    def plot_2d_comp(self, comp1, comp2, target=None, train_perc=0.9, colors=['pink', 'green', 'cyan'], alpha=0.25, savefile=None):
